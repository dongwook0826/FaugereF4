...
structure SymbProcStruct
  ...
  lmon_done (not_top : last_mon ≠ ⊤) :
    mo.toSyn.invFun (last_mon.get (by
      cases last_mon with
      | top =>
        simp_all only [Submodule.carrier_eq_coe, ne_eq, not_true_eq_false]
      | coe lmon =>
        simp_all only [Submodule.carrier_eq_coe, ne_eq, WithTop.coe_ne_top, not_false_eq_true]
        rfl
    )) ∈ done_mons
...
noncomputable def symbolic_preprocess_step {σ K : Type*}
  ...
  have b_mem_mon_H : b ∈ mon_H := by
    ...
    have : ∃ b'' ∈ mon_H, mo.toSyn b'' = b' := by
      ...
      · unfold b
        ...
        have : b' = mo.toSyn.toEmbedding (mo.toSyn.invFun b') := by
          simp only [AddEquiv.toEquiv_eq_coe, Equiv.invFun_as_coe, AddEquiv.coe_toEquiv_symm,
    ...
    have : b = b'' := by
      unfold b
      rw [← syn_b'']
      simp only [AddEquiv.toEquiv_eq_coe, Equiv.invFun_as_coe, EquivLike.coe_symm_apply_apply]
    subst this
  ...
  have b_notin_last_done : b ∉ sps.done_mons := by
    ...
    simp_all only [AddEquiv.toEquiv_eq_coe, Finset.mem_map_equiv, AddEquiv.coe_toEquiv_symm,
      Finset.mem_sdiff, Equiv.invFun_as_coe, not_false_eq_true]
  have b_in_done : b ∈ done_mons := by
    unfold done_mons
    simp only [Finset.singleton_union, Finset.mem_insert, true_or]
  have nd_lt_b (ndm : σ →₀ ℕ) : -- proof of nd_lt_lmon
    ...
    have lem_2 :
      ...
      have :
        Finset.map mo.toSyn.toEmbedding (mon_H \ done_mons) =
          (Finset.map mo.toSyn.toEmbedding ((mon_H \ sps.done_mons).erase b)) := by
        simp_all only [AddEquiv.toEquiv_eq_coe, Finset.map_erase, Equiv.coe_toEmbedding,
          EquivLike.coe_coe]
      have :
    ...
    have lem_3 :
      ...
      rw [← lem_2]
      have : mo.toSyn ndm = mo.toSyn.toEmbedding ndm := by
  ...
  if h_div_able : SubG ≠ ∅
    then
      ...
      have f_not_0 : f ≠ 0 := by
        simp_all only [Equiv.invFun_as_coe, AddEquiv.toEquiv_eq_coe, AddEquiv.coe_toEquiv_symm,
          Finset.singleton_union, Finset.mem_insert, or_false, Finset.mem_sdiff, not_or, and_imp,
          ne_eq, Finset.filter_eq_empty_iff, not_forall, Decidable.not_not,
          Finset.mem_filter, true_and, mon_H, b, b', done_mons, SubG, f]
        intro a
      ...
      have lmf_div_b : lmf ≤ b := by
        ...
        have : leading_monomial mo f ≤ b := by
          simp_all only [Equiv.invFun_as_coe, AddEquiv.toEquiv_eq_coe, AddEquiv.coe_toEquiv_symm,
            Finset.singleton_union, Finset.mem_insert, or_false, Finset.mem_sdiff, not_or, and_imp,
            ne_eq, Finset.filter_eq_empty_iff, not_forall, Decidable.not_not, Finset.mem_filter,
            true_and, mon_H, b, b', done_mons, SubG, f]
        apply nonzero_lm'_div_impl_lm_div
        exact this
      let mulf := f * MvPolynomial.monomial (b - lmf) (1 : K)
      have lm_mulf :
        leading_monomial' mo mulf
          (by
            simp_all only [Equiv.invFun_as_coe, AddEquiv.toEquiv_eq_coe, AddEquiv.coe_toEquiv_symm,
              Finset.singleton_union, Finset.mem_insert, or_false, Finset.mem_sdiff, not_or,
              and_imp, ne_eq, Finset.filter_eq_empty_iff, not_forall, Decidable.not_not,
              Finset.mem_filter, true_and, mul_eq_zero, MvPolynomial.monomial_eq_zero, one_ne_zero,
              or_self, not_false_eq_true, mon_H, b, b', done_mons, SubG, f, lmf, mulf]
          ) =
          b := by
        unfold mulf
        rw [
          lm'_monmul_commute mo f f_not_0 _ _
      ...
      let H := {mulf} ∪ sps.H
      {
        ...
        zero_not_mem_H := by
          ...
          · simp_all only [Equiv.invFun_as_coe, AddEquiv.toEquiv_eq_coe, AddEquiv.coe_toEquiv_symm,
              Finset.singleton_union, Finset.mem_insert, or_false, Finset.mem_sdiff, not_or,
              and_imp, ne_eq, Finset.filter_eq_empty_iff, not_forall, Decidable.not_not,
              Finset.mem_filter, true_and, zero_eq_mul, MvPolynomial.monomial_eq_zero, one_ne_zero,
              or_self, not_false_eq_true, mon_H, b, b', done_mons, SubG, f, lmf, mulf]
        ...
        done_sub_H := by
          have : monomial_set sps.H ⊆ monomial_set H := by
            ...
            have : sps.H ⊆ H := by
              unfold H
              simp only [Finset.singleton_union, Finset.subset_insert]
        ...
        nd_lt_lmon := by
          ...
          have mon_H_union : monomial_set H = monomial_set {mulf} ∪ monomial_set sps.H := by
            unfold H
            exact monomial_set_union_distrib {mulf} sps.H
          ...
          | inl ndm_mem_mulf =>
            ...
            have syn_ndm_le_b : mo.toSyn ndm ≤ b' := by
              ...
              rw [← lm_mulf]
              apply Finset.le_max'
              simp_all only [ne_eq, Finset.mem_union, MvPolynomial.mem_support_iff,
                Finset.mem_map_equiv, EquivLike.coe_symm_apply_apply, not_false_eq_true]
        ...
        div_then_cont_mult := by
          ...
          · apply Or.inr
            ...
            · exists b - lmf
              subst m
              constructor
              · simp only [Finset.singleton_union, mul_comm, Finset.mem_insert, true_or, H, mulf]
          ...
          · have : m ∈ sps.done_mons := by
              ...
              simp_all only [Finset.mem_sdiff, and_imp, ne_eq, Finset.singleton_union,
                Finset.mem_insert, false_or]
    ...
    else
      {
        ...
        nd_lt_lmon := by
          unfold mon_H at nd_lt_b
          simp_all only [Finset.mem_sdiff, and_imp, ne_eq, Decidable.not_not, WithTop.coe_lt_coe,
        ...
        div_then_cont_mult := by
          ...
          · have : m ∈ sps.done_mons := by
              unfold done_mons at m_done
              simp_all only [Finset.mem_sdiff, and_imp, ne_eq, Decidable.not_not,
                Finset.singleton_union, Finset.mem_insert, false_or]
...
lemma sps_last_mon_decr {σ K : Type*}
  ...
  have b'_lt_lmon : b' < sps.last_mon := by -- termination proof
    ...
    · have b_mem : b ∈ mon_H \ sps.done_mons := by
    ...
    SymbProcStruct σ K mo G H0 :=
  {
    ...
    H_sub_ideal_G := by
      ...
      have : Ideal.span {g} ≤ Ideal.span G := by
        apply Ideal.span_mono
        simp only [Set.singleton_subset_iff, SetLike.mem_coe, hgG]
      apply this
      simp only [Ideal.mem_span_singleton, dvd_mul_right]
    zero_not_mem_H := hH0
    done_mons := ∅
    done_sub_H := by simp only [Finset.empty_subset]
    last_mon := ⊤
    lmon_done := by
      simp only [ne_eq, not_true_eq_false, AddEquiv.toEquiv_eq_coe, Equiv.invFun_as_coe,
        AddEquiv.coe_toEquiv_symm, Finset.notMem_empty, imp_self]
    nd_lt_lmon := by simp only [Finset.sdiff_empty, WithTop.coe_lt_top, implies_true]
    div_then_cont_mult := by
      simp only [Finset.notMem_empty, not_exists, not_and, IsEmpty.forall_iff, implies_true]
  }
...
lemma span_ge_symb_proc_red_0_induction {σ K : Type*}
  ...
  | h μ'_ IH =>
      ...
      · unfold f' at α_supp_f'
        ...
        | inl α_supp_f =>
          ...
          simp only [AddEquiv.toEquiv_eq_coe, Equiv.invFun_as_coe, AddEquiv.coe_toEquiv_symm,
            AddEquiv.apply_symm_apply]
          apply Finset.le_max'
          simp_all only [ne_eq, AddEquiv.toEquiv_eq_coe, Equiv.invFun_as_coe,
            AddEquiv.coe_toEquiv_symm, not_false_eq_true, forall_const,
            MvPolynomial.mem_support_iff, Finset.mem_map_equiv]
        | inr α_supp_n =>
    ...
    | inl n_mem_N' =>
      ...
      | inr f'_ne_0 =>
        have IH' :=
          IH _ (f'_eq_0_or_lmf'_lt_lmf f'_ne_0) f' f'_mem_V f'_ne_0
            (by simp only [AddEquiv.toEquiv_eq_coe, Equiv.invFun_as_coe,
        ...
        have : f = f' + MvPolynomial.C (f.coeff (leading_monomial' mo n n_ne_0)) * n := by
          simp only [MvPolynomial.smul_eq_C_mul, sub_add_cancel, f']
    ...
    | inr n_not_mem_N' =>
      ...
      have f''_eq_0_or_lmf''_lt_lmf (f''_ne_0 : f'' ≠ 0) :
        ...
        · unfold f'' at β_supp_f''
          ...
          | inl β_supp_f =>
            ...
            simp only [AddEquiv.toEquiv_eq_coe, Equiv.invFun_as_coe, AddEquiv.coe_toEquiv_symm,
              AddEquiv.apply_symm_apply]
            apply Finset.le_max'
            simp_all only [ne_eq, not_false_eq_true, forall_const, AddEquiv.toEquiv_eq_coe,
              Equiv.invFun_as_coe, AddEquiv.coe_toEquiv_symm, exists_prop, mul_eq_zero,
              MvPolynomial.monomial_eq_zero, or_self, not_exists, not_and, forall_exists_index,
              and_imp, MvPolynomial.mem_support_iff, Finset.mem_map_equiv]
          | inr β_supp_αg =>
            rw [
              ← mul_one
                (f.coeff (leading_monomial' mo f f_ne_0) *
        ...
        · simp only [MvPolynomial.mem_support_iff, MvPolynomial.coeff_sub, ne_eq,
          ...
          conv_rhs =>
            ...
            rw [
              lm'_monmul_commute mo g g_ne_0 α 1
                (by simp only [ne_eq, one_ne_zero, not_false_eq_true])]
            rw [add_comm]
      ...
      | inr f''_ne_0 =>
        ...
        have IH' :=
          IH _ lmf''_lt_lmf f'' f''_mem_V f''_ne_0
            (by simp only [AddEquiv.toEquiv_eq_coe, Equiv.invFun_as_coe,
              EquivLike.coe_symm_apply_apply])
        have key :=
          mul_basis_add_red_0 mo (G ∪ N')
...
lemma pair_set_insert {τ : Type*} [LinearOrder τ] (e : τ) (S : Finset τ) (heS : e ∉ S) :
  ...
  ext ⟨p, q⟩
  simp_all only [pair_set, Finset.mem_filter, Finset.mem_product, Finset.mem_insert,
    Finset.product_singleton, gt_iff_lt, Finset.singleton_product, Finset.union_assoc,
    Finset.mem_union, Finset.mem_map, Function.Embedding.coeFn_mk, Prod.mk.injEq, ↓existsAndEq,
    true_and, exists_eq_right_right]
  aesop
...
lemma pair_set_insert_card {τ : Type*} [LinearOrder τ] (e : τ) (S : Finset τ) (heS : e ∉ S) :
  ...
  have disj_1 : Disjoint (pair_set S) ((S.filter (· < e)) ×ˢ {e}) := by
    ...
    · simp only [Finset.notMem_empty, Finset.product_singleton, Finset.mem_inter, Finset.mem_map,
        Finset.mem_filter, Function.Embedding.coeFn_mk, Prod.mk.injEq, ↓existsAndEq, true_and,
        IsEmpty.forall_iff]
  have disj_2 : Disjoint (pair_set S ∪ (S.filter (· < e)) ×ˢ {e}) ({e} ×ˢ (S.filter (· > e))) := by
    ...
    · simp only [Finset.notMem_empty, Finset.product_singleton, gt_iff_lt,
        Finset.singleton_product, Finset.mem_inter, Finset.mem_union, Finset.mem_map,
        Finset.mem_filter, Function.Embedding.coeFn_mk, Prod.mk.injEq, ↓existsAndEq, true_and,
        exists_eq_right_right, IsEmpty.forall_iff]
...
lemma pair_set_card_ind {τ : Type*} [LinearOrder τ] :
  ...
  | zero =>
    ...
    simp only [pair_set, hS, Finset.product_empty, Finset.filter_empty, Finset.card_empty,
      zero_tsub, mul_zero, Nat.zero_div]
  | succ n ih =>
    intro S hS
    have S_nonempty : S.Nonempty := by
      rw [← Finset.one_le_card, hS]
    ...
    rw [
      pair_set_insert_card maxS (S.erase maxS)
        (by
    ...
    rw [card_1, card_2, hS]
    cases n with
    | zero => simp only [zero_tsub, mul_zero, Nat.zero_div, add_zero, zero_add, tsub_self]
    | succ m =>
      apply @Nat.eq_of_mul_eq_mul_right _ 2 _ (by simp only [Nat.ofNat_pos])
...
-- the definition requires more than 400000 heartbeats
...
noncomputable def F4_step {σ K : Type*} [Finite σ] [DecidableEq σ] [Field K] [DecidableEq K]
  ...
  have L_sub_monmul_G :
    ...
    have fi_mem_G : fi ∈ f4s.G := by simp only [Fin.getElem_fin, List.getElem_mem, fi]
    have fj_mem_G : fj ∈ f4s.G := by simp only [Fin.getElem_fin, List.getElem_mem, fj]
    ...
    (Fin f4s.size × Fin f4s.size) ↪ (Fin (f4s.size + N'.card) × Fin (f4s.size + N'.card)) :=
  {
    ...
    inj' := by
      simp only [Function.Injective, Prod.mk.injEq, Fin.castAdd_inj, and_imp, Prod.forall,
        forall_apply_eq_imp_iff, forall_eq', Prod.mk.eta, implies_true]
  }
  ...
  have i_pairs_card : i_pairs.card = size * (size - 1) / 2 := by
    have : (@Finset.univ (Fin (f4s.size + N'.card)) _).card = size := by
      simp only [Finset.card_univ, Fintype.card_fin, size]
    rw [pair_set_card (@Finset.univ (Fin (f4s.size + N'.card)) _), this]
  {
    ...
    zero_not_mem_G := by
      ...
      · have : 0 ∉ N := ge_L'.zero_not_mem_SO
        ...
        intro ⟨h0N, _⟩
        exact h0N
    zero_not_mem_G' := by -- indeed equivalent to `zero_not_mem_G`
      intro i hi
    ...
    i_pairs_lt := by
      ...
      intro i j
      simp only [pair_set, Finset.univ_product_univ, Finset.mem_filter, Finset.mem_univ, true_and]
    ...
    sat_buchberger := by
      ...
      have i_eq_i' : i.val = i'.val := by
        rw [← ext_ij'_eq_ij.1]
        simp only [Fin.val_castAdd]
      have j_eq_j' : j.val = j'.val := by
        rw [← ext_ij'_eq_ij.2]
        simp only [Fin.val_castAdd]
      have i_ran : i.val < f4s.G.length := by
        rw [i_eq_i', f4s.G_len_eq_size]
        simp only [Fin.is_lt]
      have j_ran : j.val < f4s.G.length := by
        rw [j_eq_j', f4s.G_len_eq_size]
        simp only [Fin.is_lt]
      have len_le : f4s.G.length ≤ (f4s.G ++ N'.toList).length := by
        simp only [List.length_append, Finset.length_toList, le_add_iff_nonneg_right, zero_le]
      have get_i_eq :
      ...
      have spoly_ij_eq : spoly_ij = sij - sji := by
        simp only [S_poly, Fin.getElem_fin, get_i_eq, get_j_eq, spoly_ij, sij, spair_ij, sji]
...
lemma f4s_moni_ipcard_lex_decr {σ K : Type*} [Finite σ] [DecidableEq σ] [Field K] [DecidableEq K]
    ...
    (Fin f4s.size × Fin f4s.size) ↪ (Fin (f4s.size + N'.card) × Fin (f4s.size + N'.card)) :=
  {
    ...
    inj' := by
      ...
      simp only [Prod.mk.injEq, Fin.castAdd_inj, and_imp, Prod.forall, forall_apply_eq_imp_iff,
        forall_eq', Prod.mk.eta, implies_true]
  }
  let size := f4s.size + N'.card
  let G := f4s.G ++ N'.toList
  let i_pairs := pair_set (@Finset.univ (Fin (f4s.size + N'.card)) _)
  ...
  have f4s_moni_ipcard_lex_decr' :
    ...
    | inl N'_empty =>
      ...
      have N'_card_0 : N'.card = 0 := by rw [N'_empty]; simp only [Finset.card_empty]
      apply Or.inr
      constructor
      · unfold lmi' lmi G
        simp only [N'_list_empty, List.append_nil]
      · rw [Finset.card_sdiff_of_subset i_pairs_proc_subs,
        ...
        have hc1 : i_pairs.card = f4s.i_pairs.card := by
          have : size = f4s.size := by simp only [N'_card_0, add_zero, size]
          ...
          · unfold lmi' monomial_ideal
            have lm'_n_mem :
              ...
              · simp only [List.mem_append, Finset.mem_toList, G]
                exact Or.inr hnN'
              · simp only [lm_coe_lm' mo n (ne_of_mem_of_not_mem hnN' zero_not_mem_N'),
                  WithBot.some_eq_coe]
  ...
  have G_eq : (F4_step mo I f4s full_proc).G = G := by
    unfold F4_step
    simp only [Fin.getElem_fin, Lean.Elab.WF.paramLet, G, N', L', L, i_pairs_new, ip_to_spair,
      symb_proc_L, N, ge_L']
  have card_eq :
    ...
    simp only [ne_eq, Fin.getElem_fin, Lean.Elab.WF.paramLet, N', L', L, i_pairs_new, ip_to_spair,
      symb_proc_L, N, ge_L']
  rw [G_eq, card_eq]
...
noncomputable def F4_init {σ K : Type*} [Finite σ] [DecidableEq σ] [Field K] [DecidableEq K]
  ...
  have supp_G_le_size : F.toList.toFinsupp.support = Finset.range F.toList.length := by
    ...
    constructor
    · intro hx
      simp_all only [List.getD_eq_getElem?_getD, ne_eq, Finset.length_toList, Finset.mem_filter,
        Finset.mem_mk, Multiset.mem_range]
    · intro hx
      ...
      exact ne_of_mem_of_not_mem this hF
  {
    size := F.card
    G := F.toList
    G_len_eq_size := by simp only [Finset.length_toList]
    G_inj_on_supp := by
      intro i hi j hj hij
    ...
    i_pairs_proc := ∅
    i_pairs_proc_subs := by simp only [Finset.empty_subset]
    i_pairs_lt := by
      intro ⟨i, j⟩
      unfold init_i_pairs pair_set
      simp_all only [Finset.length_toList, Finset.univ_product_univ, Finset.mem_filter,
        Finset.mem_univ, true_and, Fin.val_fin_lt]
    i_pairs_card := by
      have : (@Finset.univ (Fin F.card) _).card = F.card := by
...
theorem F4_groebner {σ K : Type*} [Finite σ] [DecidableEq σ] [Field K] [DecidableEq K]
  ...
  have i_ne_j : i ≠ j := by
    by_contra i_eq_j
    subst i f1 f2
    have spoly_eq_0 : spoly = 0 := by simp only [S_poly, S_pair, sub_self, spoly]
  ...
  | inl i_lt_j =>
    have ii_lt_jj : ii < jj := by simp only [Fin.mk_lt_mk, i_lt_j, ii, jj]
    ...
    exact spoly_ne_0
  | inr i_gt_j =>
    have jj_lt_ii : jj < ii := by simp only [Fin.mk_lt_mk, i_gt_j, jj, ii]
